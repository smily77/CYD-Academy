/*
  23c_3D_Car_BNO055 - Interaktives 3D-Auto mit Orientierungssensor

  Dieses Beispiel zeigt ein 3D-Auto (blocky/retro-style), das virtuell im Raum
  "schwebt". Das CYD-Board funktioniert wie eine Kamera, die du um das Auto
  herum bewegen kannst. Drehe das Board nach rechts → siehst du das Auto von
  rechts. Kippe es nach vorne → siehst du das Auto von oben. Genau wie bei
  einem echten Modell-Auto, das du von allen Seiten betrachten kannst!

  Funktionen:
  - BNO055 Sensor im NDOF-Modus (Gyro + Accelerometer + Magnetometer)
  - 3D-Auto-Rendering mit Perspektive (Karosserie + Kabine)
  - Verschiedenfarbige Teile (Karosserie, Fenster)
  ...
  WICHTIG - Sensor-Modus:
  - Nutzt NDOF mode (Gyro + Accelerometer + Magnetometer)
  - Z-Achse kann indoor leicht driften (Magnetfeld-Störungen normal)
  - X/Y-Achsen bleiben stabil (nutzen Schwerkraft)

  Lernziele:
  - Komplexe 3D-Modelle erstellen (mehrere verbundene Quader)
  - 3D-Grafik-Programmierung (Rotation, Projektion)
  - Quaternionen & Sensor-Fusion verstehen
...
Face carFaces[12] = {
  ...
  Face(6, 2, 3, 7, CAR_BODY, 0),        // Front (Z+)
  Face(5, 4, 0, 1, CAR_BODY, 0),        // Heck (Z-)
  Face(7, 3, 0, 4, CAR_BODY, 0),        // Links (X-)
  Face(6, 5, 1, 2, CAR_BODY, 0),        // Rechts (X+)

  // Kabine/Dach (oberer Block)
  Face(10, 9, 8, 11, CAR_CABIN_TOP, 0), // Kabine-Boden (meist unsichtbar)
  Face(12, 13, 14, 15, CAR_CABIN_TOP, 0), // Dach (Y+)
...
void setup() {
  ...
  Serial.println("Setup complete! Move the board around the virtual car (board = camera).");
  lcd.fillScreen(COLOR_BG);
}
...
void drawFaces() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (showWireframe) {
      ...
      // Kanten (schwarz)
      if (renderToSprite) {
        cubeSprite.drawLine(p[0].x, p[0].y, p[1].x, p[1].y, CAR_EDGE);
        cubeSprite.drawLine(p[1].x, p[1].y, p[2].x, p[2].y, CAR_EDGE);
        cubeSprite.drawLine(p[2].x, p[2].y, p[3].x, p[3].y, CAR_EDGE);
        cubeSprite.drawLine(p[3].x, p[3].y, p[0].x, p[0].y, CAR_EDGE);
      } else {
        lcd.drawLine(p[0].x, p[0].y, p[1].x, p[1].y, CAR_EDGE);
        lcd.drawLine(p[1].x, p[1].y, p[2].x, p[2].y, CAR_EDGE);
        lcd.drawLine(p[2].x, p[2].y, p[3].x, p[3].y, CAR_EDGE);
        lcd.drawLine(p[3].x, p[3].y, p[0].x, p[0].y, CAR_EDGE);
      }
    }
  }
}
...
void drawSplashScreen() {
  ...
  // Einfaches Auto-Symbol (Seitenansicht)
  int cx = 120, cy = 100;

  // Karosserie (unterer Block)
  lcd.fillRect(cx - 40, cy, 80, 25, CAR_BODY);
  lcd.drawRect(cx - 40, cy, 80, 25, COLOR_TEXT);

  // Kabine (oberer Block)
  lcd.fillRect(cx - 20, cy - 20, 40, 20, CAR_BODY);
  lcd.drawRect(cx - 20, cy - 20, 40, 20, COLOR_TEXT);

  // Fenster
  lcd.fillRect(cx - 17, cy - 17, 15, 14, CAR_WINDOW);
  lcd.fillRect(cx + 2, cy - 17, 15, 14, CAR_WINDOW_FRONT);

  // Räder (2 sichtbar)
  lcd.fillCircle(cx - 20, cy + 25, 8, COLOR_TEXT_DIM);
  lcd.fillCircle(cx + 20, cy + 25, 8, COLOR_TEXT_DIM);
  lcd.drawCircle(cx - 20, cy + 25, 8, COLOR_TEXT);
  lcd.drawCircle(cx + 20, cy + 25, 8, COLOR_TEXT);
  ...
  lcd.setTextColor(COLOR_TEXT);
  lcd.setTextDatum(MC_DATUM);
  lcd.drawString("3D CAR", 120, 180);

  // Version
  lcd.setTextSize(1);
  lcd.setTextColor(COLOR_TEXT_DIM);
  lcd.drawString("Example 23c", 120, 210);
  lcd.drawString("Rotate to explore", 120, 230);
}
